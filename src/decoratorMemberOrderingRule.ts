...
import { mapDefined, flatMap, dedent } from 'tslint/lib/utils';
...
enum MemberKind {
    ...
    publicDecoratedAccessor,
    protectedDecoratedAccessor,
    privateDecoratedAccessor,
    ...
    publicAbstractAccessor,
    protectedAbstractAccessor,
    privateAbstractAccessor,
...
export class Rule extends Lint.Rules.AbstractRule {
    ...
    public static metadata: Lint.IRuleMetadata = {
        ruleName: 'decorator-member-ordering',
    ...
    public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
        ...
        try {
            ...
            console.warn(`Warning: ${this.ruleName} - ${(e as Error).message}`);
            return [];
        }
        return this.applyWithWalker(new DecoratorMemberOrderingWalker(sourceFile, this.ruleName, options));
    }
}

class DecoratorMemberOrderingWalker extends Lint.AbstractWalker<Options> {
    ...
    private checkMembers(members: ts.NodeArray<Member>) {
        ...
        for (let i = 1; i < members.length; i++) {
            ...
            if (rank < prevRank) {
                ...
                const prevName = decoratedNameString(prevMember);
                    ...
                    if (this.options.alphabetize && a.name !== undefined && b.name !== undefined) {
                        ...
                        const getSetDiff =
                            a.kind === ts.SyntaxKind.GetAccessor && b.kind === ts.SyntaxKind.SetAccessor
                                ? -1
                                : a.kind === ts.SyntaxKind.SetAccessor && b.kind === ts.SyntaxKind.GetAccessor
                                ? 1
                                : 0;
...
function getMemberKind(member: Member): MemberKind | undefined {
    ...
    switch (member.kind) {
        ...
        case ts.SyntaxKind.MethodSignature:
            if (member.name !== undefined && nameString(member.name).startsWith('ng')) {
